...
data = data.drop(['WANDSCHIEFE_RAHMEN_P1_NUM2'], axis = 1)
data = data.drop(['Ssp_Randkonstruktion'], axis = 1)
data = data.drop(['DEB_I_LAGERUNG_UEBERBAU2'], axis = 1)
#data = data.drop(['n0_3'], axis = 1)
...
lin_opt = gp_minimize(model_eval_linear, [Integer(1, 20), Real(0, 40)], n_calls = 60)
...
def model_eval_nn(params):
    ...
    class model(torch_nn.Module):
        def __init__(self) -> None:
            ...
            self.network = torch_nn.Sequential(
                torch_nn.Linear(int(train_data.shape[1]), int(params[0])), 
                torch_nn.ReLU(), 
                torch_nn.Dropout(p = float(params[1])),
                torch_nn.Linear(int(params[0]), 1)
            )
            self.double()
        def forward(self, x):
    ...
    nn = model().to(device)
    nn.apply(he_init)
    optimizer = optim.Adam(nn.parameters(), lr=float(params[3]), weight_decay=float(params[4]))
    init_nn = deepcopy(nn.state_dict())
    init_opt = deepcopy(optimizer.state_dict())
    """
    class RMSLELoss(torch_nn.Module):
        def __init__(self):
            super().__init__()
    ...
    def train(features, labels):
        ...
        dataset = TensorDataset(torch.from_numpy(features_train).to(device), torch.from_numpy(labels).to(device))
        dataloader = DataLoader(dataset, batch_size=int(params[2]), shuffle=True)
        for i in range(100):
...
nn_opt = gp_minimize(model_eval_nn, [Integer(1, 8192), Real(0, 0.9999), Integer(1, 128), Real(0.0001, 0.1), Real(0, 20)], n_calls = 25)
...
xgboost_opt = gp_minimize(model_eval_xgboost, [Real(0, 20), Real(0.01, 0.5), Integer(3, 10), Integer(100, 1100), Real(0.5, 1), Integer(1, 10), Real(0, 5), Real(0, 5)], n_calls = 60)
