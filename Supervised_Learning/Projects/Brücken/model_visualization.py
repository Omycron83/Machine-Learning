...
nn_model.adam_iterated(train_data[:int(train_data.shape[0] * 0.8)], labels[:int(train_data.shape[0] * 0.8)], NN.MSE, dropout= [nn_opt[1]], batchsize = nn_opt[2], alpha = nn_opt[3], _lambda = nn_opt[4], iterations = 250)
...
np.savetxt("D:\\Damian\\PC\\Python\\ML\\Supervised_Learning\\Projects\\Brücken\\LinRegr.csv", np.hstack((data_id,  pred_linregr )), delimiter=",")
...
np.savetxt("D:\\Damian\\PC\\Python\\ML\\Supervised_Learning\\Projects\\Brücken\\NN.csv", np.hstack((data_id, pred_nn)), delimiter=",")

print("Starting saving xgboost:")
xgboost_model = xgboost.XGBRegressor(gamma = xgboost_opt[0], learning_rate = xgboost_opt[1], max_depth = xgboost_opt[2], n_estimators = xgboost_opt[3], n_jobs = 16, objective = 'reg:squarederror', subsample = xgboost_opt[4], scale_pos_weight = 0, reg_alpha = xgboost_opt[6], reg_lambda = xgboost_opt[7], min_child_weight = xgboost_opt[5])
xgboost_model.fit(train_data, labels)
pred_xgboost = xgboost_model.predict(data_unknown)
np.savetxt("D:\\Damian\\PC\\Python\\ML\\Supervised_Learning\\Projects\\Brücken\\XGBoost.csv", np.hstack((data_id, pred_xgboost )), delimiter=",")
