...
lin_opt = [0]
nn_opt = [453, 0.48377941566444677, 117, 0.0006685591228068569, 0.01999257672168958]
xgboost_opt = [0.0, 0.04491353111733209, 5, 100, 0.7761690258201821, 10, 5.0, 4.424087360747103]
...
#Saving the values (including overfitting):
print("Starting saving linear:")
lin_model = LinRegr.linear_regression(train_data.shape[1], _lambda = lin_opt[0])
lin_model.ridge_normal_eq(train_data, labels)
pred_linregr = lin_model.predict(train_data)
print(pred_linregr[:25].T)
...
nn_model = NN.cont_feedforward_nn(train_data.shape[1], [nn_opt[0]], NN.ReLU, NN.ReLUDeriv, NN.output, NN.MSE_out_deriv, 1)
nn_model.adam_iterated(train_data, labels, NN.MSE, dropout= [nn_opt[1]], batchsize = nn_opt[2], alpha = nn_opt[3], _lambda = nn_opt[4], iterations = 1200)
...
pred_xgboost = xgboost_model.predict(train_data)
print(pred_xgboost[:25].T)
