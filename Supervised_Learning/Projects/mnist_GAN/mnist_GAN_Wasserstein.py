...
#building a gan, but we are using the wasserstein loss as the ordinary loss just sucks (vanishing gradient)
...
def discriminatorBackwards(train_X, train_Y, _lambda, theta1, theta2, theta3, predValues, z2, z1, a2, a1):
    ...
    z = np.zeros(predValues.shape)
    z[np.where(rounds(train_Y) == 0)] += np.average(predValues[np.where(rounds(train_Y) == 1)])
    z[np.where(rounds(train_Y) == 1)] += np.average(predValues[np.where(rounds(train_Y) == 0)])
    delta3 = predValues - z
...
def discriminatorForward(train_X, train_Y, _lambda, theta1, theta2, theta3, q):
    ...
    a2 = np.hstack((np.ones((np.shape(z2)[0],1)), ReLU(z2)))
    #last step
    predValues = con(a2)@con(np.transpose(theta3))
    J = 0#np.sum(predValues[np.where(rounds(train_Y) == 0)]) #/ len(np.where(rounds(train_Y) == 0)) - np.sum(predValues[np.where(rounds(train_Y) == 1)]) / len(np.where(rounds(train_Y) == 1))
    ...
    return J, predValues, z2, z1, a2, a1
    ...
    for i in range(1,iterations+1):
        ...
        for j in range(len(train_Y)//batchsize + 1):
            if len(batchY) > batchsize:
                ...
                #
                J, predValues, z2, z1, a2, a1 = discriminatorForward(curr_batch, curr_batchY, _lambda, theta1, theta2, theta3, q)
...
def main():
    ...
    #
    J, predValues, z2, z1, a2, a1 = discriminatorForward(test_X, test_y, _lambda, theta1, theta2, theta3, 0)
    print("With an error of", J)
    fig = plt.figure()
