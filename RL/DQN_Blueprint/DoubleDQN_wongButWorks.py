    ...
    class replay_memory:
        ...
        def update_history(self, history, discount_rate, update_network):
            for k in range(len(history)):
                ...
                #(Better) Option 2: we use the bellman equation aka temporal difference learning, thus making the "right" value: r + discount_rate (gamma) * update_output_max(s', a')
                #If we arent at the terminal state, we use the temporal difference reward
                if k != len(history) - 1:
                    update_network.forward_propagation(history[k + 1][0], np.zeros((1, update_network.len_output)), NN.MSE)
                    self.Output_history[-1][history[k][3]] = history[k][2] + discount_rate * np.amax(update_network.layers_for[-1].z[0])
                #If we are at the terminal state, we use just the terminal reward                                                                       
                else:
                    self.Output_history[-1][history[k][3]] = history[k][2]
